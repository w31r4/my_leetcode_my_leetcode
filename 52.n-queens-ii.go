...
func totalNQueens(n int) (answer int) {
	...
	dfs = func(level int) {
		...
		for i := 0; i < n; i++ {
			//第 level 行，第 i 列
			if columns[i] == 0 && !mainDiag[level-i+n-1] && !sideDiag[level+i] {
				// copy(temp, board)
				// operateQueen(level, i, n, board, true)
				columns[i] = 1
				mainDiag[level-i+n-1] = true
				sideDiag[i+level] = true
				dfs(level + 1)
				columns[i] = 0
				mainDiag[level-i+n-1] = false
				sideDiag[i+level] = false
				// copy(board, temp)
